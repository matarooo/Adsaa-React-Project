...
export default function Post(postData: { slug: string; title: any; excerpt: any; author: any; category: any; image: any; date: any; readTime: any }) {
    const {  slug: postSlug , title , excerpt , author , category , image , date , readTime} = postData
  return (
   <article className="group card overflow-hidden" style={{animationDelay: '0ms'}}>
          <Link className="block" to={`/blog/${postSlug}`} data-discover="true"><div className="relative h-52 overflow-hidden">
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 text-sm text-neutral-500 mb-3">
                <span className="flex items-center gap-1"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{readTime}</span><span className="w-1 h-1 bg-neutral-600 rounded-full" /><span>{date}</span>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-[#262626]">
                <div className="flex items-center gap-3">
                  <img alt="سالم أحمد" className="w-9 h-9 rounded-full object-cover ring-2 ring-[#262626]" src={author.avatar} />
                  <div>
